...
$p->print(&RfamWWWConfig::header("Query Rfam text"));
$p->print("<center><SPAN class=normallargetext>Results for query '<SPAN class=normalbluetext>$terms</SPAN>'</SPAN></center><p>");
...
#}
$p->print("<center><SPAN CLASS=normaltext>Matches to documentation in the selected databases with links back to Rfam</SPAN></center><p>\n");
if (@results) {
    ...
    $p->print("<table border=1 cellpadding=5 cellspacing=0 align=center bgcolor=$RfamWWWConfig::rfamcolour><tr bgcolor=#000070  ><th class=whitetableheader>Family</th><th  class=whitetableheader>Description</th></tr>\n");
    foreach my $res (@results) {
	if (not $res->{'des'}) {
	    $res->{'des'} = $res->{'acc'};
	}
	$p->print("<tr><td > <a href=\"$RfamWWWConfig::getacc?".$res->{'acc'}."\">".$res->{'name'}."</a></td><td class=normaltext>".$res->{'des'}."</td></tr>");
	$p->break;
    }
    $p->print("</table>\n");
}
else {
    $p->print("<center><span class=normallargetext>There were no Rfam families hit</center>");
}
#$p->print("<BR><hr noshade  color=#000070 size=2>\n<P>");
$p->print("<P><P>");
$p->print(&RfamWWWConfig::footer() );
$p->break;
