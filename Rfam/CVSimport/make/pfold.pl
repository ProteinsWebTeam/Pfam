...
if( `uname` !~ /linux/i ) {
    &usage();
    print STDERR "FATAL: this only runs on linux boxes at the moment\n";
    exit(1);
}
...
#    $fh -> print("$pfold_bindir2/cluster.tcsh $$.fa");
    system("$pfold_bindir2/cluster.tcsh $$.fa") and die "failed while running [$pfold_bindir2/cluster.tcsh $$.fa]";
#    $fh -> close();
...
#    $fh -> print("$pfold_bindir/addparen /tmp/$alnfile.$$.res.col | $pfold_bindir/col2fasta > /tmp/$alnfile.$$.out.fa\n");
    system("$pfold_bindir/findphyl $pfold_bindir/scfg.rate /tmp/$alnfile.$$.col > /tmp/$alnfile.$$.nj.col\n") and die;
    system("$pfold_bindir/mltree $pfold_bindir/scfg.rate /tmp/$alnfile.$$.nj.col > /tmp/$alnfile.$$.ml.col\n") and die;
    system("$pfold_bindir/scfg --robust=$robust --treeinfile $pfold_bindir/article.grm /tmp/$alnfile.$$.ml.col > /tmp/$alnfile.$$.res.col\n") and die;
    system("$pfold_bindir/addparen /tmp/$alnfile.$$.res.col | $pfold_bindir/col2fasta > /tmp/$alnfile.$$.out.fa\n") and die;
#    $fh -> close;

    open( O, ">/tmp/$alnfile.$$.out.fa2" ) or die;
    open( S, "/tmp/$alnfile.$$.out.fa" ) or die;
    ...
    if( $split ) {
	system "sreformat -u -r --gapsym '.' --mingap stockholm /tmp/$alnfile.$$.out.fa2 | sed 's/GC_SS_cons/GC SS_cons/g' > $alnfile" and die;
    }
    else {
	system "sreformat -u -r --gapsym '.' --mingap stockholm /tmp/$alnfile.$$.out.fa2 | sed 's/GC_SS_cons/GC SS_cons/g'" and die;
    }
}
