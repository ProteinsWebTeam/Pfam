...
=head2 fastMapPfamA2PDB

 Title    : fastMapPfamA2PDB
 Usage    : &fastMapPfamA2PDB($rdb, $pdb_id, $coos_format);
 Function : Takes an RDB ro object and gets the sws_acc, domain names/s-e for the given pdb id.
 Returns  : hash pdb_id, chain, seq_id, domain_index, "domain_acc/start_end"
 Args     : rdb handle, pdb ID
...
sub fastMapPfamA2PDB {
    my ($sth, $pdb, $coos_format) = @_;
    die "Your pdb identifier, [$pdb] is incorrect, expecting in the form 2abl\n" if ($pdb !~ /\S{4}/);
    $sth->execute($pdb);
   my $res = $sth->fetchall_arrayref;
    ...
    my %mapping;
#$mapping{$pdb}{$$seq[0]}{$$seq[1]}{$domain_index}{"$acc/$s-$e"}++;
    if($coos_format eq "PDB"){
	foreach my $domainMap (@{$res}){
	    my $chain = $$domainMap[2];
	    $chain = "_" unless ($chain =~ /\S+/);
	    $chain = "_" if ($chain eq "NULL");
	    $mapping{$pdb}{$chain}{$$domainMap[3]}{$$domainMap[0]}{$$domainMap[1]."/".$$domainMap[4]."-".$$domainMap[5]}++;
	}
    }else{
	foreach my $domainMap (@{$res}){
	    my $chain = $$domainMap[2];
		$chain = "_" unless ($chain =~ /\S+/);
	    $mapping{$pdb}{$chain}{$$domainMap[3]}{$$domainMap[0]}{$$domainMap[1]."/".$$domainMap[6]."-".$$domainMap[7]}++;	
	}
    }
    return %mapping;
}
