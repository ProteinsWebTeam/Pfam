...
# finalise paths
export PATH=/software/bin:/sbin:/usr/bin:$PATH
...
DAEMON0=$PFAMWEBROOT/PfamBackend/scripts/hmmerDequeue0.pl
DAEMON1=$PFAMWEBROOT/PfamBackend/scripts/hmmerDequeue1.pl
NAME=hmmerDequeue
...
case "$1" in
  ...
  start)
    ...
    start-stop-daemon --start --quiet --pidfile /var/lock/${NAME}0.pid \
      -c pfamweb --exec $PERLBIN --startas $DAEMON0
    echo "${NAME}0."

    start-stop-daemon --start --quiet --pidfile /var/lock/${NAME}1.pid \
      -c pfamweb --exec $PERLBIN --startas $DAEMON1
    echo "${NAME}1."
    ;;
  ...
  stop)
    ...
    start-stop-daemon --stop --signal 15 --quiet --pidfile /var/lock/${NAME}0.pid \
      -c pfamweb --exec $PERLBIN --startas $DAEMON0
    echo "${NAME}0."

    start-stop-daemon --stop --signal 15 --quiet --pidfile /var/lock/${NAME}1.pid \
      -c pfamweb --exec $PERLBIN --startas $DAEMON1
    echo "${NAME}1."
    ;;
