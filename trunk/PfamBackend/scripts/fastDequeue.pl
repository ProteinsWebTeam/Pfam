   ...
   sleep 2;
    ...
    if ( $ref->{'job_type'} eq "pfamb" ) {
      ...
      open ( FA, '>' . $qsout->tmpDir . '/' . $ref->{job_id} . '.fa' ) 
        or $error .= "Could not open file for writing: $!";

      print FA ">UserSeq\n" if $ref->{stdin} !~ /^>/;
      print FA $ref->{stdin}."\n";

      close FA
        or $error .= "Could not close fasta file: $!";
      ...
      $cmd = $ref->{command} 
             . ' --in '     . $ref->{job_id} . '.fa'
             . ' --tmpdir ' . $qsout->tmpDir
             . ' --data '   . $qsout->rfamDataFileDir
             . ' --acc '    . $ref->{options};  # needs to bring the accession

      if ( ( $qsout->rfcpus || 0 ) > 1 ) {
        $cmd .= ' --mpi ' . $qsout->rfcpus;
      }
