...
# jt6 20060810 WTSI
#
# $Id: Interpro.pm,v 1.1 2009-11-27 11:40:34 pg6 Exp $
...
package iPfamWeb::Controller::Search::Keyword::Interpro;
...
sub process : Path {
  ...
  $c->log->debug( 'Search::Keyword::Interpro::process: text querying table pfamA using: |' .
  ...
  while(my $r = $rs->next ){
    ...
    push @$results,{
      ...
      'desc'          =>  $desc,
      'interpro_id'   =>  $r->get_column( 'interpro_id'),
      'type'          =>  'family'
    };
    $seen->{ $acc }++;
  }
  $c->stash->{ hits } = $total_hits;
  $c->log->debug( "Search::Keyword::interpro::process: the total number of results are  ".scalar( @$results ) );
  $c->stash->{ results } = $results;
  $c->stash->{ template } = 'components/blocks/search/keyword/interpro.tt';
}
