...
package PfamBase::Roles::SpeciesTree;

=head1 DESCRIPTION

This is a role to add methods for generating interactive or text
representations of a species tree. 
...
medium trees (numSpecies < denyInteractiveLimit) can be time consuming to
generate as interactive trees, so we refuse to generate them unless we have a
flag set in the request

=item 

large trees (numSpecies < denyAllLimit) are too big for the interactive view,
so we refuse to generate the interactive tree but will generate a text
representation

=item 

very large trees (numSpecies > denyAllLimit) can't be touched, so we refuse to
generate either interactive or text trees
