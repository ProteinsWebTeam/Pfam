...
PfamBase::Roles::Root - base role for the Xfam Root controllers

=cut

package PfamBase::Roles::Root;

=head1 DESCRIPTION

This role applies a set of useful actions to the Xfam website controllers.
...
sub index : Private {
  ...
  $c->log->debug('PfamBase::Roles::Root::index: generating site index')
    if $c->debug;
}
...
sub announcements : Local {
  ...
  unless ( $response->is_success ) {
    $c->log->warn( "PfamBase::Roles::Root::announcements: couldn't retrieve blog content from |"
  ...
  unless ( defined $feed ) {
    $c->log->warn( "PfamBase::Roles::Root::announcements: couldn't parse blog content" )
      if $c->debug;
    $c->res->status( 204 );
    return;
  }
  ...
  foreach my $entry ( $feed->entries ) {
    ...
    if ( defined $cookie ) {
      # $c->log->debug( "PfamBase::Roles::Root::announcements: cookie shows timestamp "
      #                 . $cookie->value . '; entry issued at '
      #                 . $issued ) if $c->debug;
      
      if ( $issued > $cookie->value ) {
        # $c->log->debug( "PfamBase::Roles::Root::announcements: post is newer than cookie; showing" )
        #   if $c->debug;
        # $entries->{$issued} = $entry;
      }
      # else {
      #   $c->log->debug( "PfamBase::Roles::Root::announcements: cookie is newer than post; NOT showing" )
      #     if $c->debug;
      # }
      
    }
    else { 
      # $c->log->debug( "PfamBase::Roles::Root::announcements: no $type cookie found; adding post "
      #                 . $entry->id )
      #   if $c->debug;
      $entries->{$issued} = $entry;
    }
  ...
  foreach my $issued ( reverse sort keys %$entries ) {
    ...
    $c->log->debug( "PfamBase::Roles::Root::announcements: adding post $i" )
      if $c->debug;
    
    # TODO should make the maximum number of posts into a configuration value
    if ( $i >= 3 ) { 
      $c->log->debug( "PfamBase::Roles::Root::announcements: reached limit for posts" )
        if $c->debug;
      last;
    }
  ...
  if ( scalar keys %$entries ) {
    $c->log->debug( "PfamBase::Roles::Root::announcements: found some posts; handing off to template" )
      if $c->debug;
    $c->stash->{template} = 'pages/announcements.tt';
  }
  else {
    $c->log->debug( "PfamBase::Roles::Root::announcements: found NO posts; returning 204" )
      if $c->debug;
    $c->res->status( 204 );
  }
...
sub end : Private {
  ...
  if( scalar @{ $c->error } ) {
    $c->log->warn( 'PfamBase::Roles::Root::end: found some errors from previous methods' )
      if $c->debug;
