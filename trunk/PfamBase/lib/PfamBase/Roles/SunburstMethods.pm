...
PfamBase::Roles::SunburstMethods - role to add various sunburst-related
methods to the family page controller

=cut

package PfamBase::Roles::SunburstMethods;
...
before [ qw( accessions_GET_html 
  ...
  unless ( $c->stash->{jobId} ) {
    $c->log->debug( 'SunburstMethods::before "accessions_GET*": bad job id' )
      if $c->debug;
  }
};
...
sub accessions_GET_html {
  ...
  if ( $c->stash->{jobId} ) {
    $c->log->debug( "SunburstMethods::accessions_GET_html: building tool window for job $job_id (" . $c->stash->{jobId} .')' )
      if $c->debug;
  }
  else {
    $c->log->debug( 'SunburstMethods::accessions_GET_html: bad job id' )
      if $c->debug;
...
sub accessions_GET {
  ...
  unless ( $c->stash->{jobId} ) {
    $c->log->debug( 'SunburstMethods::accessions_GET: no job id' )
      if $c->debug;
...
sub fasta : Chained( 'sunburst' )
  ...
  unless ( $c->stash->{jobId} ) {
    $c->log->debug( 'SunburstMethods::fasta: bad job id' )
      if $c->debug;
  ...
  unless ( $fasta and
           $fasta =~ m/^>/ ) {
    $c->log->debug( 'SunburstMethods::fasta: failed to generate fasta file' )
      if $c->debug;
...
sub align_POST {
  ...
  if ( $c->stash->{errorMsg} ) {
    $c->log->debug( 'SunburstMethods::align_POST: no job; returning 404' )
      if $c->debug;
  ...
  if ( $c->forward('queue_alignment_job') ) {
    $c->log->debug( 'SunburstMethods::align_POST: successfully queued alignment job' )
      if $c->debug;
  }
  else {
    $c->log->debug( 'SunburstMethods::align_POST: FAILED to successfully queue alignment job' )
      if $c->debug;
  }
...
sub align_GET {
  ...
  unless ( $jobId =~ m/^([A-F0-9]{8}\-([A-F0-9]{4}\-){3}[A-F0-9]{12})$/i ) {
    $c->log->debug( 'SunburstMethods::align_GET: bad job id' )
      if $c->debug;
  ...
  unless ( defined $job ) {
    $c->log->debug( 'SunburstMethods::align_GET: no job; returning 404' )
      if $c->debug;
  ...
  if ( $job->status eq 'DONE' ) {
    $c->log->debug( 'SunburstMethods::align_GET: job complete' )
      if $c->debug;

    $c->res->status( 200 ); # OK

    if ( $poll_only ) {
      $c->log->debug( 'SunburstMethods::align_GET: this is a polling request; returning just status 200' )
        if $c->debug;
      $c->res->body( 'DONE' );
    }
    else {
      $c->log->debug( 'SunburstMethods::align_GET: not a polling request; returning alignment' )
        if $c->debug;
  ...
  $c->log->debug( 'SunburstMethods::align_GET: job failed or deleted; returning 500 error' )
    if $c->debug;
...
sub store_accessions : Private {
  ...
  $c->log->debug( 'SunburstMethods::store_accessions: found ' . scalar @$accessions_list .
  ...
  #   $c->log->debug( 'SunburstMethods::store_accessions: not a valid accessions string' )
  #     if $c->debug;
  ...
  if ( $@ ) {
    # oops...
    $c->log->error( "SunburstMethods::store_accessions: error from query: |$@|" )
      if $c->debug;
...
sub retrieve_accessions : Private {
  ...
  $c->log->debug( "SunburstMethods::retrieve_accessions: looking up job ID, $job_id" )
    if $c->debug;
  ...
  unless ( $job ) {
    $c->log->debug( 'SunburstMethods::retrieve_accessions: no row for that job id' )
      if $c->debug;
...
sub queue_alignment_job : Private {
  ...
  if ( defined $exists and
    ...
    if ( $c->forward('enqueue_alignment') ) {
      $c->log->debug( 'SunburstMethods::queue_alignment_job: alignment job '
  ...
  $c->log->debug( 'SunburstMethods::queue_alignment_job: sequence search submitted')
    if $c->debug;
...
sub enqueue_alignment : Private {
  ...
  try {
    ...
    $c->log->debug( 'SunburstMethods::enqueue_alignment: problem submitting alignment job: $_' )
      if $c->debug;
  ...
  $c->log->debug( 'SunburstMethods::enqueue_alignment: submitted job |'
...
sub queue_alignment : Private {
  ...
  if ( $c->debug ) {
    $c->log->debug( 'SunburstMethods::queue_alignment: job status: ', dump( $job_status ) );
