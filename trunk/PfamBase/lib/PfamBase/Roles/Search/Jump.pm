...
PfamWeb::Roles::Jump - guess the type of entry that the user wants to see

=cut

package PfamBase::Roles::Search::Jump;
...
sub jump : Path( '/search/jump' ) {
  my ( $this, $c ) = @_;
  
  # de-taint the entry ID or accession
  my ( $entry ) = $c->req->param('entry') || '' =~ /^([\w\-_\s()\.]+)$/;
  $entry ||= '';
  $c->log->debug( "PfamBase::Roles::Search::Jump::jump: called with entry |$entry|" )
    if $c->debug;

  # strip off leading and trailing whitespace
  $entry =~ s/^\s*(.*?)\s*$/$1/;
  $c->log->debug( "PfamBase::Roles::Search::Jump::jump: trimmed entry to |$entry|" )
    if $c->debug;
  ...
  if ( $c->req->param('type') ) {
    ...
    $c->log->debug( 'PfamBase::Roles::Search::Jump::jump: looking for entry type: |'
...
sub end : Private {
  ...
  if ( $c->stash->{error} ) {
    ...
    if ( defined $c->req->param('redirect') and
      ...
      $c->log->debug( 'PfamBase::Roles::Search::Jump::end: redirect parameter set; redirecting to guessed URL' )
        if $c->debug;
