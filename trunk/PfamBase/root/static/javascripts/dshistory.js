...
* Revision date: $Date: 2008-05-16 14:58:22 $
...
var dsHistory = function() {
       ...
       var encodeURIComponent = window.encodeURIComponent; // close a reference to this function since we'#------------------------------------------------------------------------------- be calling it so often and since it will be faster than going up the scope each time
       ...
       function getEncodedWindowHash(forceRecompute) {
               ...
               // for performance, we'#------------------------------------------------------------------------------- assume that if we're doing more than 9 concats that it will be quicker to use and array and then use the .join('&') trick
       ...
       function fluxCapacitor() {
               ...
               // if the frame iteration is different or the window hash is different, we'#------------------------------------------------------------------------------- start a sequence of events to go back in time
               if (
                       !isGoingForward
                       && (
       ...
       returnObject = {
               ...
               addFunction: function(fnc, scope, objectArg) {
                       ...
                       // since moving back won't change the location hash. we'#------------------------------------------------------------------------------- use the hash cache to manually change the cache then.
                       if (browser.IE)
