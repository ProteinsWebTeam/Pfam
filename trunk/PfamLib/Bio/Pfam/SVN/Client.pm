...
sub checkFamilyExists {
  ...
  eval{
    $self->{txn}->info($url, undef, 'HEAD', $codeRef , 0 );
  };
...
sub checkAllClanFiles {
  ...
  foreach my $file (keys %{ $self->{config}->{files}->{clan} } ){
    ...
    if($@){
      #Todo, should change this to confess
       warn "$file for $clan does not exist in the respository at $url.  This is very bad\n";
    }
  }
}
...
sub log {
  ...
  my $subRef = sub {  my( $changed_paths, $revision, $author, $date, $message ) = @_;
      chomp($message);
      print STDOUT "-------------------------------------------------------------------------------\n";
      print STDOUT "revision $revision\ndate: $date;  author: $author;\n$message\n";
  };
    
  print  $self->{txn}->log($url, 1, defined($rev) ? $rev : $self->revision, 0, 0, $subRef);
  print STDOUT "===============================================================================\n";
...
sub commitFamily {
  ...
  my $cinfo;
  eval {
    $cinfo = $self->{txn}->commit($family, 1);
  };
...
sub addFamily {
  ...
  unless(-e "$dest/$newFamilyId"){
    mkdir("$dest/$newFamilyId") or confess("Could not make directory $dest/$family:[$!]\n");
  }
  
  foreach my $file (@{ $self->{config}->mandatoryFamilyFiles }){
    copy("$family/$file", "$dest/$newFamilyId/$file") or confess("Failed to copy $family/$file to $dest/$newFamilyId/$file");
  }

  #Now add the families.
  eval{
    $self->{txn}->add("$dest/$newFamilyId", 1 ); 
  };
  ...
  my $cinfo;
  eval {
    $cinfo = $self->{txn}->commit("$dest/$newFamilyId", 1);
  };
...
sub update {
  ...
  foreach my $file ( @{ $self->{config}->mandatoryFamilyFiles }){
    $self->{txn}->update($family."/".$file, $self->{config}->svnRevision, 1);
  }
...
sub addClan {
  ...
  unless(-e "$dest/$newClanId"){
    mkdir("$dest/$newClanId") or confess("Could not make directory $dest/$newClanId:[$!]\n");
  }

#TODO - Change this to come from the config  
#  foreach my $file (@{ $self->{config}->mandatoryFamilyFiles }){
  foreach my $file (qw(CLANDESC)){ 
    copy("$clan/$file", "$dest/$newClanId/$file") or confess("Failed to copy $clan/$file to $dest/$newClanId/$file");
  }

  #Now add the Clan.
  eval{
    $self->{txn}->add("$dest/$newClanId", 1 ); 
  };
...
sub newFamilyLocation {
  my ( $self ) = @_;
  my $newFamilyUrl = $self->{config}->svnRepos.$self->{config}->svnNewFamilies;
  return($newFamilyUrl);
}
...
sub addPFCILog {
  ...
  my $commit = sub {
    ...
    #See if we have a default messge to use.
    if(-s ".default".$$."pfci"){
        open(M, ".default".$$."pfci") or die "Could not open .default".$$."pfci";
        while(<M>){
          $message .= $_;  
        }
        close(M);  
...
sub addPFKILLLog {
  ...
  my $commit = sub {
    ...
    #See if we have a default messge to use.
    if(-s ".default".$$."pfkill"){
        open(M, ".default".$$."pfkill") or die "Could not open .default".$$."pfci";
        while(<M>){
          $message .= $_;  
        }
        close(M);  
    }else{
      die "No pfkill message as to why the family is being removed from pfam!\n";
    }
...
sub addPFMOVELog {
  ...
  my $commit = sub {
    ...
    #See if we have a default messge to use.
    if(-s ".default".$$."pfmove"){
        open(M, ".default".$$."pfmove") or die "Could not open .default".$$."pfci";
        while(<M>){
          $message .= $_;  
        }
        close(M);  
...
sub addPFNEWMOVELog {
  ...
  my $commit = sub {
    ...
    if(-s ".default".$$."pfnewmove"){
        open(M, ".default".$$."pfnewmove") or die "Could not open .default".$$."pfci";
        while(<M>){
          $message .= $_;  
        }
        close(M);  
...
sub addCLMOVELog {
  ...
  my $commit = sub {
    ...
    #See if we have a default messge to use.
    if(-s ".default".$$."clmove"){
        open(M, ".default".$$."clmove") or die "Could not open .default".$$."clci";
        while(<M>){
          $message .= $_;  
        }
        close(M);  
...
sub addCLKILLLog {
  ...
  my $commit = sub {
    ...
    #See if we have a default messge to use.
    if(-s ".default".$$."clkill"){
        open(M, ".default".$$."clkill") or die "Could not open .default".$$."clkill";
        while(<M>){
          $message .= $_;  
        }
        close(M);  
...
sub addAUTORMCLLog {
 ...
 my $commit = sub {
    my $passmessage = shift; #Scalar reference passed by svn binding
    $$passmessage .= "AUTORMCL:Automatically removed clan from family DESC file";
 };
...
sub addCLCILog {
  ...
  my $commit = sub {
    ...
    #See if we have a default messge to use.
    if(-s ".default".$$."clci"){
        open(M, ".default".$$."clci") or die "Could not open .default".$$."clci:[$!]\n";
        while(<M>){
          $message .= $_;  
        }
        close(M);  
