package Bio::Pfam::Clan::Clan_Web;
...
sub auto_clan {
   ...
   if( defined $value) {
       $self->{'auto_clan'} = $value;
   }
  
   return $self->{'auto_clan'};
}

sub clan_description {
   my ($self,$value) = @_;

   if( defined $value) {
       $self->{'clan_description'} = $value;
   }
  
   return $self->{'clan_description'};
}
...
sub clan_author {
   ...
   if( defined $value) {
       $self->{'clan_author'} = $value;
   }
  
   return $self->{'clan_author'};
}
...
sub clan_comment {
   ...
   if( defined $value) {
       $self->{'clan_comment'} = $value;
   }
  
   return $self->{'clan_comment'};
}
sub  updated {
   my ($self,$value) = @_;
...
sub member_families {
   my ($self,$line) = @_;
#print "LINE: $line <BR>";
   push(@{$self->{'member_families'}},$line);

}

sub each_member_families {
   my ($self,@args) = @_;
# print "WOWSER " .@{$self->{'member_families'}} . " <P>";
   return @{$self->{'member_families'}};
}
...
sub full{
   ...
   open(_ALIGN,"gunzip -c $dir/full/$acc.full.gz |") || $self->throw("For entry object [$id], got no valid directory for ALIGN alignment [$dir/full/$acc.full.gz] $!");
