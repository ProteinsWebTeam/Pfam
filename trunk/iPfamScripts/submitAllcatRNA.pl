...
#
# Author     : rdf
# Maintainer : $Id: submitAllcatRNA.pl,v 1.1 2008-11-23 19:55:11 rdf Exp $
# Created    : 
# Modified   : $Date: 2008-11-23 19:55:11 $
# 

=head1 DESCRIPTION

  Script to submit blocks of RNA chain finds to the WTSI farm.    
...
my $logger = Log::Log4perl->get_logger('iPfamSubmitAllcatRNA');
...
my $exec =  'cd /lustre/scratch1/sanger/pfam/ipfam/RNA; /nfs/team71/pfam/rdf/iPfamScripts/catalogRNAChains.pl ';
$exec .= " -numberPdbs $numberPdbsPerJob";
$exec .= " -page \$\{LSB_JOBINDEX\}";
$exec .= " -h $rdb_host -rdb $rdb_name -port $rdb_port -p $rdb_pass";
$exec .= " >  rnaChains.\$\{LSB_JOBINDEX\} ";
$logger->debug("LSF:  -J ipfamRNA[1-$numJobs], -q $lsfQueue -R select[mem>1500 && $sqlResource<$maxLoad && type==X86_64] rusage[$sqlResource=10:mem=1500]");
