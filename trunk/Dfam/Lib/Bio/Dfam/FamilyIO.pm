...
sub loadDfamFromLocalFile {
  ...
  #my $DESC = to_DESCDfam($params{'DESC'});
  #print STDERR $DESC;
  #my $TABLE = to_TableDfam($params{'TABLE'});
  #print STDERR $TABLE;
  #my $HMM = to_HMMDfam($params{'HMM'});
  #print STDERR $HMM;
  
  #my $SEED = to_AlignDfam($params{'SEED'});
  #print STDERR $SEED;
  #exit;
  #my $TABLE = to_TableDfam($params{'TABLE'});
  #print STDERR $TABLE;
 #my $DESC = to_DESCDfam($params{'DESC'});
  ...
  my $famObj = 'Bio::Dfam::Family'->new( ('HMM'   => to_HMMDfam($params{'HMM'}),
                                          'DESC'  => to_DESCDfam($params{'DESC'}),
                                          'TABLE' => to_TableDfam($params{'TABLE'}),
                                          
                                           'SEED'  => to_AlignDfam($params{'SEED'}),
                                           'ALIGN' => to_AlignDfam($params{'SEED'})
));
...
sub loadDfamAFromSVN {
  ...
  my $famObj = $self->loadDfamAFromLocalFile( $family, $dir, 'svn' );
  return $famObj;
}
...
"DESC file format for link to EC is wrong $acc is not a valid accession\n"
  ...
  my $desc = 'Bio::Dfam::Family::DESC'->new(%params);
  return $desc;
...
sub writeEmptyDESC {
  ...
  my %desc = (
   # ID    => 'ShortName',
    ...
    SM    => 'hmmsearch -Z ' . $self->{config}->dbsize . ' -E 1000 HMM dfamseq',
    TP    => 'Family'
  );
...
sub deleteDfamAInRDB {
  ...
  $dfamDB->deleteDfamA( $family, $comment, $forward, $user );
