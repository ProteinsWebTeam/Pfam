...
Bio::Dfam::SVN::Client - a modules that enables access to the Pfam SVN

=cut

package Bio::Dfam::SVN::Client;
...
sub checkFamilyExists {
  ...
  if ($@) {
    ...
    if ( $config and $config->location eq 'JFRC' ) {
      my $dfamDB = Bio::Dfam::LiveDBManager->new( %{ $config->dfamlive } );

      my @dead =
        $dfamDB->getSchema->resultset("DeadFamilies")
