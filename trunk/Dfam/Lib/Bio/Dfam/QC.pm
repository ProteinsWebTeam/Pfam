#
# QC - an attempt to bring all the dfam quality control
...
=head2 passesAllFormatChecks
 ...
 Usage    : &QC::passesAllFormatChecks(familyObject, directoryName, inNewFlag)
 Function : Runs all format check routines from this module,
            and warns about any errors
 Returns  : 1 if all is OK, 0 if not
 Args     : Bio::Dfam::Family object, directory containing the family, true flag if family is new
...
# Usage    : &QC::local_dirs_overlap( \@dirs )
...
#
# Title    : name_clashes
# Usage    : &QC::name_clashes("family_id","ignore")
...
=head2 compareAlignToScores
 ...
 Usage    : &QC::compare_align_to_scores("family_id")
...
=head2 more_in_align_than_seed
 ...
 Usage    : &QC::more_in_align_than_seed("family_id")
...
=head2 mulFormatIsOK
 ...
 Usage    : &QC::mul_format_is_OK("file","family_id")
...
=head2 nameFormatIsOK
 ...
 Usage    : &QC::name_format_is_OK("family_id")
...
sub nameFormatIsOK {
  ...
  if ( $family =~ /^[A-Za-z0-9_\-]{1,15}$/ ) {
    ...
    warn "Your Dfam name contains invalid characters and/or is too long\n";
    return 0;
  }
}
...
=head2 sequenceChecker
 ...
 Usage    : QC::sequenceChecher($familyObject)
 Function : Checks the sequences in the SEED and ALIGN are valid.
 Returns  : 1 if there are errors, 0 if the SEED is fine
 Args     : A Bio::Dfam::Family::DfamA object 
...
sub sequenceChecker {
  ...
  unless ( $famObj->isa("Bio::Dfam::Family::DfamA") ) {
    confess("A Bio::Dfam::Family::DfamA object was not passed in");
  }
  ...
  foreach my $aln (qw(SEED ALIGN)) {
    foreach my $seq ( $famObj->$aln->each_seq ) {
      ...
      Bio::Dfam::SeqFetch::addSeqToVerify( $seqName,
        $seq->start, $seq->end, $str_ali, \%allseqs );
    }
  }

  my $verified_seq = Bio::Dfam::SeqFetch::verifySeqs( \%allseqs,
    $CONFIG->dfamseqLoc . "/dfamseq" );

  if ( $verified_seq == $count ) {
    print STDERR "\n--- All sequences match the dfamseq database ---\n\n";
  }
  else {
...
=head2 family_overlaps_with_db
 ...
 Usage    : &QC::family_overlaps_with_db("family_id", \@ignore, 1)
...
sub family_overlaps_with_db {
  my ( $family, $ignore_ref, $endpoints_opt, $dfamDB, $famObj, $compete ) = @_;
  my ( %ignore, @overlaps );
  ...
  my $nestedRef = $dfamDB->getNestedDomain( $famObj->DESC->AC );

  if ($nestedRef) {
    foreach my $n (@$nestedRef) {
      $$ignore_ref{$n}++;

      my $clan = $dfamDB->getClanDataByDfam($n);
      if ( $clan and $clan->auto_clan->clan_acc ) {
        my $clanMem = $dfamDB->getClanMembership( $clan->auto_clan->clan_acc );
        foreach my $fam (@$clanMem) {
          $$ignore_ref{ $fam->auto_dfama->dfama_acc }++;
        }
      }
    }
  }
  ...
  if ( $famObj->DESC->NESTS ) {
    foreach my $nesting ( @{ $famObj->DESC->NESTS } ) {
      ...
      my $clan = $dfamDB->getClanDataByDfam( $nesting->{dom} );
      if ( $clan and $clan->auto_clan->clan_acc ) {
        my $clanMem = $dfamDB->getClanMembership( $clan->auto_clan->clan_acc );
        foreach my $fam (@$clanMem) {
          $$ignore_ref{ $fam->auto_dfama->dfama_acc }++;
  ...
  if ( $famObj->DESC->CL ) {
    ...
    my $clanMem = $dfamDB->getClanMembership( $famObj->DESC->CL );
    foreach my $fam (@$clanMem) {
      $$ignore_ref{ $fam->auto_dfama->dfama_acc }++;
      my $nestedRef =
        $dfamDB->getNestedDomain( $fam->auto_dfama->dfama_acc );
...
#We now allow overlaps between envelopes.
  ...
  my %overlaps;
  $dfamDB->getOverlapingFullDfamRegions( \%regions, \%overlaps );
  $dfamDB->getOverlapingSeedDfamRegions( \%regions, \%overlaps );


  my $dfamoutRegions;
  if($compete){
    $dfamoutRegions = $famObj->PFAMOUT->eachHMMSeq;  
  }
 ...
 REGION:
      foreach my $overRegion ( @{ $region->{overlap} } ) {
        ...
        if($region->{ali} eq 'FULL' and $compete){
         if(_compete($seqAcc, $region, $overRegion, $dfamDB, $dfamoutRegions, $famObj->DESC->CL)){
          next REGION;  
         }
        }
...
=head2 noMissing
  ...
  Title    : noMissing
  Usage    : Bio::Dfam::QC::nomissing($oldFam, $newFam, $name) 
  Function : Checks that all of the sequences in the old family are present
           : in the updated version.
  Args     : Bio::Dfam::Family::DfamA objects representing the old and new families and
...
=head2 noFragsInSeed
 ...
 Title    : noFragsInSeed
 Usage    : Bio::Dfam::QC::noFragsInSeed($family)
...
=head2 checkFamilyFiles
 ...
 Usage    : &QC::check_current_family_files("family_id")
...
=head2 verifyEdits
  ...
  Title    : verifyEdits
  Usage    : Bio::Dfam::QC::verifyEdits($family, $familyObj);
  ...
  Args     : name of the family, a Bio::Dfam::Family::DfamA object
  Returns  : 1/0, success or failure
...
=head2 checkNestings 
  ...
  Title    : checkNestings
  Usage    : Bio::Dfam::QC::checkNestings($family, $familyObj);
...
sub checkNestings {
  ...
  if ( $famObj->DESC->NESTS and scalar( @{ $famObj->DESC->NESTS } ) ) {
    unless ( $famObj->SEED->match_states_string )
      ...
      #and $famObj->SEED->match_states_string->isa('Bio::Dfam::OtherReg') )
    {
      warn
...
sub checkDESCSpell {
  ...
  #
  unless ($familyIO) {
    $familyIO = Bio::Dfam::FamilyIO->new;
  }
...
sub _compete {
  my( $seqAcc, $region, $overRegion, $dfamDB, $dfamoutRegions, $clanAcc) = @_; 
  
  my $skip = 0;
  my $thisEvalue;
  foreach my $seq (@$dfamoutRegions){
    #Find the sequence
  ...
  if($clanAcc and $clanAcc =~ /CL\d{4}/){
    ...
    #less than this one!
    my $seqRegions = $dfamDB->findLowerEvalueRegion($seqAcc, $region, $clanAcc, $thisEvalue);  
    if($seqRegions > 0){
      $skip = 1;  
    }
  }
  ...
  unless( $skip == 1 ) {
    ...
    my $otherClanAcc;
    my $cRS = $dfamDB->getClanDataByDfam($overRegion->{family});
    if($cRS){
    ...
    if($otherClanAcc and $otherClanAcc =~ /CL\d{4}/){
      ...
      #Look up to see if there are any regions in the database with an E-value
      #less than this one!
      my $seqRegions = $dfamDB->findLowerEvalueRegion($seqAcc, $overRegion, $otherClanAcc);  
      if($seqRegions > 0){
        $skip = 1;  
      }
    }
