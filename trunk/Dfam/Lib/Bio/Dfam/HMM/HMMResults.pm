...
package Bio::Dfam::HMM::HMMResults;
...
sub addHMMSeq {
  ...
  unless($hmmSeq->isa('Bio::Dfam::HMM::HMMSequence')){
    die 'Trying to add a non Bio::Dfam::HMM::HMMSequence object';  
  }
...
sub addHMMUnit {
  ...
  unless($hmmUnit->isa('Bio::Dfam::HMM::HMMUnit')){
    die "Trying to add an non-Bio::Dfam::HMM::HMMUnit\n";
  }
...
sub remove_overlaps_by_clan {
    ...
    my $new = Bio::Dfam::HMM::HMMResults->new;
	...
	unless($o) {
		...
		$new->addHMMSeq( Bio::Dfam::HMM::HMMSequence->new( {  name       => $self->seqs->{$unit->name}->name,
