...
my ($all,$duf,$dufnum,$ci,$viewseed,$not_associated,$ignoreoverlaps,$author,$overwrite,$embl);
&GetOptions('all!' => \$all,
	    'duf!' => \$duf,
	    'dufnum=s' => \$dufnum,
	    'overwrite!' => \ $overwrite,
...
Arguments:
         -duf <DUF number> - Add DUF information using user defined number
                           - will default to latest number
         -overwrite        - Will attempt to overwrite DESC file with new annotation
         -ci               - Interactively ask if you want to pfnew family
         ...
         -author <authors> - Replace default author list
         -embl             - Experimental option to identify interesting UniProt
                           - entries from their EMBL cross refs
EOF
exit 1;
}
...
my @boring_pmids;
open (PMID, "/nfs/users/nfs_a/agb/boring_pmids") or die "cannot open file /nfs/users/nfs_a/agb/boring_pmids";
while(<PMID>){
    if (/^(\S+)\s+/){
	push (@boring_pmids,$1);
    }
...
# This is a frequency has for embl xrefs
my %embl_freq;
open (EMBL, "/nfs/users/nfs_a/agb/freq_embl") or die "cannot open file /nfs/users/nfs_a/agb/freq_embl";
while(<EMBL>){
    if (/^(\S+)\s+(\d+)/){
	$embl_freq{$1}=$2;
    }
...
FAMILY: foreach my $dir (sort @dir_list){
    ...
    if ($duf){
	if ($type eq "Protein"){
	    $comment.=$pad."This family of proteins is found in";
	} else {
	    $comment.=$pad."This domain family is found in";
	}
    } else {
	if ($type eq "Protein"){
 ...
 #    if ($duf){$pad=" ";}
	...
	if ($db_refs{$link}){
		...
		if ($embl){
			...
			my $return_acc=interesting_embl($acc);
			if ($return_acc){
			    print STDERR "EMBL $acc ($embl_freq{$return_acc}) Direct submission\n";
			    print ANN "EMBL $acc ($embl_freq{$return_acc}) Direct submission\n";
			} else {
			    #print STDERR "EMBL $acc BORING\n";
			    #print ANN "EMBL $acc BORING\n";
			}
		    }
		    next;
		} else {
	    ...
	    if ($link eq "EnsemblBacteria"){next;}
	    if ($link eq "EnsemblPlants"){next;}
	    if ($link eq "EnsemblMetazoa"){next;}
	    if ($link eq "EnsemblProtists"){next;}
	    if ($link eq "EnsemblFungi"){next;}
	    if ($link eq "Ensembl"){next;}
	    if ($link eq "WormBase"){next;}
	    foreach my $acc (keys %{$db_refs{$link}}){
...
# Returns 1 if EMBL accession is in the direct submission set
sub interesting_embl {
    my $acc=shift @_;

    if ($acc =~ /^([DJKLMSUVXYZ]\d{5});/){
#	if ($1 =~ /D|J|K|L|M|S|U|V|X|Y|Z/){
	    #print STDERR "$acc is interesting\n";
	    return $1;
#	}
    } 
	...
	my $prefix=$2;
	#die "Got here $acc";
	if ($prefix =~ /AB|AF|AJ|AM|AY|DH|DK|DQ|EF|EU|FJ|FM|FR|GQ|GU|HE|HF|HG|HM|HQ/){
	    #print STDERR "$acc is interesting\n";
	    return $embl_acc;
	}
    }
