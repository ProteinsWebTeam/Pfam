...
# init script for the dequeuer that runs jobs to align sequences in the Rfam
# sunbursts
# jgt 20140425 EBI
...
my $options = {
  ...
  lsb_stop     => '$syslog',
  lsb_sdesc    => 'Dequeuer for running Rfam sunburst alignment jobs',
  lsb_desc     => 'A queue management daemon that runs jobs that align sequences selected via the sunbursts in the Rfam website',

  program      => "$INIT_DIR/PfamScripts/search/ebi_dequeuer.pl",
  program_args => [ 'rfam_align' ],

  pid_file     => "$INIT_DIR/run/rfalign_queue.pid",
