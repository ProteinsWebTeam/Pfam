...
# init script for the dequeuer that runs the Pfam-A searches
# jgt 20140417 EBI
...
my $options = {
  ...
  lsb_stop     => '$syslog',
  lsb_sdesc    => 'Dequeuer for running Pfam-A interactive searches',
  lsb_desc     => 'A queue management daemon that runs the Pfam-A interactive search queue',

  program      => "$INIT_DIR/PfamScripts/search/ebi_dequeuer.pl",
  program_args => [ 'pfama' ],

  pid_file     => "$INIT_DIR/run/pfama_queue.pid",
