...
sub writeCM {
	my ($self, $rfam_acc, $client, $location) = @_;
	my $famObj = $self->loadRfamFromSVN($rfam_acc, $client);
	$famObj->CM->cmHeader;
	$famObj->CM->cmBody;
	$famObj->CM->hmmBody;
	$famObj->CM->hmmHeader;
	$famObj->CM->rawcm;
	$famObj->CM->match_pair_node;
	$famObj->CM->is_calibrated;
	my %cmHeader = %{$famObj->CM->cmHeader};
	my @cmBody = @{$famObj->CM->cmBody};
	my %hmmHeader = %{$famObj->CM->hmmHeader};
	my @keyarr = qw( VERSION NAME STATES NODES CLEN W ALPH RF CONS MAP DATE COM PBEGIN PEND WBETA QDBBETA1 QDBBETA2 N2OMEGA N3OMEGA ELSELF NSEQ EFFN CKSUM NULL EFP7GF ECMLC ECMLI ECMGC ECMGI CM)	;
	my %hash =( 'VERSION' => $cmHeader{'version'},
				'NAME' => $cmHeader{'name'},
				'ACC' => $rfam_acc,
				'DESC' => 'desc',
				'STATES' => $cmHeader{'states'},
				'NODES' => $cmHeader{'nodes'},
				'CLEN' => $cmHeader{'clen'},
				'W' => $cmHeader{'w'},
				'ALPH' => $cmHeader{'alpha'},
				'RF' => $cmHeader{'rf'},
				...
				'DATE' => $cmHeader{'date'},
				'COM' => $cmHeader{'com'},
				'PBEGIN' => $cmHeader{'pbegin'},
				'PEND' => $cmHeader{'pend'},
			 	'WBETA' => $cmHeader{'wbeta'},
				'QDBBETA1' => $cmHeader{'qdbbeta1'},
				'QDBBETA1' => $cmHeader{'qdbbeta1'},
				'N2OMEGA' => $cmHeader{'n2omega'},
				'N3OMEGA' => $cmHeader{'n3omega'},
				'ELSELF' => $cmHeader{'elself'},
				'NSEQ' => $cmHeader{'nSeq'},
				'EFFN' => $cmHeader{'effn'},
				'CKSUM' => $cmHeader{'cksum'},
				'NULL' => $cmHeader{'null'},
				'EFP7GF' => $cmHeader{'efp7gf'},
				'ECMLC' => $cmHeader{'ecmlc'},
				'ECMGC' => $cmHeader{'ecmgc'},
				'ECMLI' => $cmHeader{'ecmli'},
				'ECMGI' => $cmHeader{'ecmgi'},
				 );
	for my $value (@keyarr) {
		if (ref $hash{$value} eq 'ARRAY') {
			print "$value\t" . join ("\t", @{$hash{$value}})  . "\n";
		} else {
