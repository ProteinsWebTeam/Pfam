...
require '/nfs/WWW/SANGER_docs/bin-offline/Website/QueueStub.pm';
#require '/nfs/WWWdev/SANGER_docs/cgi-bin/Pfam/QueueStub.pm';
use strict;
use vars qw(@ISA);
...
#    $self->wait("ended($rmid)");
 ...
 #   print STDERR "GOTJOBS EEP<P>";
   ...
   #  print STDERR "COMMAND: $be  -E 1.0 -Z 7677 $db $id.tmp $bo > $id.$i; lsrcp $id.$i XXX_subhost_XXX:XXX_lwd_XXX<P>";
     {
       'command' => "$command; /usr/bin/scp $id.$i XXX_subhost_XXX:XXX_lwd_XXX",
