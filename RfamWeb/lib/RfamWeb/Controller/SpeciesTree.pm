...
# jt6 20060410 WTSI
#
# $Id: SpeciesTree.pm,v 1.1 2008-06-19 10:10:16 jt6 Exp $
...
sub begin : Private {
  ...
  $c->forward( 'Family','get_data', [ $entry ] ) if defined $entry;
...
sub text : Local {
  ...
  if( $textTree = $c->cache->get( $cacheKey ) ) {
    ...
    if( $c->stash->{rawTree} ) {
      ...
      if( my $release = $c->stash->{relData}->rfam_release ) {
        $textTree .= "# Generated from Rfam version $release\n";
      }
...
sub buildTree : Private {
  ...
  foreach my $region ( @{ $c->stash->{regions} } ) {
    ...
    my $speciesData = { acc     => $region->rfamseq_acc,
                        species => $species,
                        tax     => \@tax };

    # flag the node if it's in the seed alignment
    $speciesData->{inSeed}++
      if $c->stash->{inSeed}->{$region->rfamseq_acc};
