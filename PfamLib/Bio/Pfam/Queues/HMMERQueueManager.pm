...
#Now POST the request and generate the search job.
		       my $response;
RETRY: for (my $post_retry = 1; $post_retry <= 5; $post_retry++) {
	       $response = $ua->post(
			       $url,
			       'content-type' => 'application/json',
			       Content        => $json->encode( \%content )
			       );
	       last RETRY if $response->is_redirect;
# sleep 5 * $retry_count;
	       $self->_log->warn("retry POST $post_retry");
	       sleep $self->_queue_spec->{submission_delay};
       }
...
#my $ebi_id = $self->_rc->search( $search_options );
	...
	my $resval = $job->update( { ebi_id  => $ebi_id,
			status  => 'RUN',
			started => \'NOW()' } );
	unless ( $resval ) {
...
#p $status;
		...
		if ( $count > $max_retry ) {
			print "_submit_interactive_hmmer_job: Jobs should have finished.....exiting\n";;
			exit(1);
		}
...
#    closed => \'NOW()' } );
		...
		if ( $count > $max_retry ) {
			print "_handle_running_hmmer_chunks: Jobs should have finished.....exiting\n";;
			last;
		}
...
#p $status;
		...
		if ( $count > $max_retry ) {
			print "_handle_interactive_hmmer_job: Jobs should have finished.....exiting\n";;
			exit(1);
		}
...
#Check again on the job status...
	...
	if($r =~ /DOCTYPE HMTL/){
		$self->_log->debug( "Strange response, expected JSON:".$r);
	}else{
		eval{
			$results = $json->decode( $r );
		};
		if($@) {
			print "Error:$@\n\n";
			print "The json string is: [$r]\n";
		}

# $results = $json->decode( $r );
		open(my $fh, '>', '/tmp/json_hmmer.txt');
		print $fh p $results;
		close $fh;
#print Dumper $results;
		my $lastIteration = pop( @{ $results->{result} } );
# uuid is ending with .1
		...
		my $json_results = from_json $results;
		open(my $fhAppend, '>>', '/tmp/json_hmmer.txt');
		print $fhAppend p $json_results;
		close $fhAppend;
