...
sub _enqueue_pending_jobs {
  ...
  JOB: foreach my $job ( @jobs ) {
    ...
    my $search_options = { database => $self->_queue_spec->{database} };
    $search_options->{evalue} = $job_options->{evalue}
      if $job_options->{evalue};
...
sub _check_running_jobs {
  ...
  JOB: foreach my $job ( @jobs ) {
    ...
    if ( $results ) {
      ...
      $self->_log->debug( "results of job $job_id:\n$results" );
...
sub _handle_batch_job {
  ...
  if ( $running_chunks_rs->count > 0 ) {
    ...
    my $num_done = $running_chunks_rs->count;
    foreach my $chunk ( $running_chunks_rs->all ) {
      ...
      if ( $chunk_status eq 'FINISHED' ) {
        my $chunk_results = $self->_rc->result( $ebi_id );
...
sub _handle_interactive_job {
  ...
  if ( $status eq 'RUNNING' ) {
    ...
    my $json_results = from_json( $self->_rc->result($ebi_id) );
    unless ( defined $json_results ) {
      $self->_log->warn( "there was a problem deserialising the results for EBI job $ebi_id" );
      return;
    }

    $results = nfreeze $json_results;
    unless ( $results ) {
      $self->_log->warn( "there was a problem freezing the results for EBI job $ebi_id" );
      return;
    }
      
    $self->_log->debug( "successfully retrieved and froze results for EBI job $ebi_id" );

  }
  else {
    $self->_log->warn( "something went wrong with job $ebi_id" );
    die 'There was a problem running your job';
  }
...
sub _submit_batch_job {
  ...
  # retrieve and split up the input sequences
  ...
  while ( scalar @sequences > 0 ) {
    # Rob wrote the next line. He's a god.
    push @chunks, join( '', map{ ">$_\n" } splice( @sequences, 0, $self->_queue_spec->{chunk_size} ) );
  }
  ...
  foreach my $chunk ( @chunks ) {
    ...
    if ( $@ ) {
      $self->_log->logwarn( "there was a problem submitting chunk $i for job "
                            . $job->job_id );
      return 0;
    }

    $self->_log->debug( "submitted batch chunk $i, given EBI job ID $ebi_id" );
    ...
    unless ( $jh_row ) {
      $self->_log->logwarn( "there was a problem storing job info for chunk $chunk for batch job "
                            . $job->job_id );
      return 0;
    }
    ...
    unless ( $rv ) {
      $self->_log->logwarn( "there was a problem storing the sequence for chunk $chunk for batch job "
                            . $job->job_id );
      return 0;
    }
