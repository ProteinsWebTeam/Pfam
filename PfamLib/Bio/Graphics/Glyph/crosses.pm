package Bio::Graphics::Glyph::crosses;
...
sub draw_component {
  ...
  if($self->feature->desc <= 0 ) {
  	$gd->line(($x1+($self->scale/2)+1),$y1,($x2-($self->scale/2)),$y2,$fg);
  }else{
    $gd->line(($x1+($self->scale/2)+1),$y2,($x2-($self->scale/2)),$y1,$fg);
  }
}
