...
sub create_alignment {
  ...
  if (($method =~m/clustal/i) || ($method =~m/clustalw/i)){
    my $optns = "-infile=tmp$$";
    system "clustalw17 $optns -output=gcg > /dev/null";
...
#    print "\# Align with t_coffee\n";
    ...
    system "/pfam/db/Pfam/software/mafft-4.20/scripts/mafft  --quiet --maxiterate 16 tmp$$.fa > tmp$$.mafft" and die;
    system("belvu98 -o selex tmp$$.mafft > tmp$$.slx");
  } elsif ($method =~ /muscle-pro/i){
    print STDERR "\n *** Muscle 3.52 is now running.  Ignore the warning below ***\n";
    # Note need to remain this way round for the statement to distinguish between the two
    system("/pfam/db/Pfam/software/muscle3.52_src/muscle -in tmp$$ -out tmp$$.fa -quiet -maxiters 2");
    system("belvu98 -o selex tmp$$.fa > tmp$$.slx");
  } elsif ($method =~ /muscle/i){
    print STDERR "\n *** Muscle 3.52 is now running.  Ignore the warning below ***\n";
    if(`uname -a` =~ /linux/i){
	system("/pfam/db/Pfam/software/muscle_linux/muscle3.52/muscle -in tmp$$ -out tmp$$.fa -quiet");
	system("/pfam/db/Pfam/software/bin/belvu_linux -o selex tmp$$.fa > tmp$$.slx");			
    }else{
	system("/pfam/db/Pfam/software/muscle3.52/muscle -in tmp$$ -out tmp$$.fa -quiet");
	system("belvu98 -o selex tmp$$.fa > tmp$$.slx");			
    }
  ...
  # Put into mul format
    ...
    while(<TMP>) {
      if (!(/^\#/)&&(!/^$/)){          # Ignore blanks and lines beginning with #.
        /^(\S+)\s+(\S+)$/; # (name) (sequence).
...
# Do alignment
    system ("clustalw17 -profile -profile1=$mask -profile2=$fasta -secstrout=both -sequences -output=gcg > /dev/null");
