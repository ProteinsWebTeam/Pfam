...
# init script for the dequeuer that runs pfam batch sequence searches against the Pfam
# maq
...
my $options = {
  ...
  pid_file     => "$INIT_DIR/monit-files/pfam_batch_queue.pid",

  fork         => 2,
  user         => 'xfm_adm',
  group        => 'xfm_pub',

  stdout_file  => ['>>', '/tmp/pfam_batch_out.log'],
  stderr_file  => ['>>', '/tmp/pfam_batch_error.log']
};
