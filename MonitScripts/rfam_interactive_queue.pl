...
# init script for the dequeuer that runs rfam seq searches
# maq
...
my $options = {
  ...
  lsb_desc     => 'A queue management daemon that runs jobs that runs interactive sequence searches against rfam',

  program      => "$PF_HOME/PfamScripts/search/ebi_dequeuer.pl",
  program_args => [ 'rfam' ],

  pid_file     => "$INIT_DIR/monit-files/rfam_interactive_queue.pid",
