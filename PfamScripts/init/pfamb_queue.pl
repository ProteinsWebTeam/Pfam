...
# init script for the dequeuer that runs the Pfam-B searches
# jgt 20140423 EBI
...
my $options = {
  ...
  lsb_stop     => '$syslog',
  lsb_sdesc    => 'Dequeuer for running Pfam-B interactive searches',
  lsb_desc     => 'A queue management daemon that runs the Pfam-B interactive search queue',

  program      => "$INIT_DIR/PfamScripts/search/ebi_dequeuer.pl",
  program_args => [ 'pfamb' ],

  pid_file     => "$INIT_DIR/run/pfamb_queue.pid",
