...
$logger->info("Seeing which sequences from pfamA_reg_seed are in the pfamseq table");
my $pfamseq_sth=$dbh->prepare("select s.pfamseq_acc from pfamA_reg_seed s left join pfamseq p on p.pfamseq_acc=s.pfamseq_acc and p.md5=s.md5 and p.seq_version=s.seq_version where p.pfamseq_acc is null");
...
while ($pfamseq_sth->fetch()) {
  $notInPfamseq{$pfamseq_acc}=1;
}


#See which sequences from pfamA_reg_seed are not in the uniprot table
$logger->info("Seeing which sequences from pfamA_reg_seed are in the uniprot table");
my $uniprot_sth=$dbh->prepare("select s.pfamseq_acc from pfamA_reg_seed s left join uniprot u on u.uniprot_acc=s.pfamseq_acc and u.md5=s.md5 and u.seq_version=s.seq_version where u.uniprot_acc is null");
...
while ($uniprot_sth->fetch()) {
  if(exists($notInPfamseq{$uniprot_acc})) {
    $delete{$uniprot_acc}=1; #It isn't in pfamseq or uniprot, so need to delete from pfamA_reg_seed
  }
}
...
$logger->info("Deleting the $num_seq accessions from pfamA_reg_seed that are not in the pfamseq or uniprot tables");
