...
my $file2 = "$status_dir/$date_of_file" . "overlaps.familyOverlaps.filtered";
...
until(-s $file1 and -s $file2) {
  ...
  $file2 = "$status_dir/$date_of_file" . "overlaps.familyOverlaps.filtered";

  $c++;
  die "Can't locate overlaps.filtered file and familyOverlaps.filtered file, no overlaps file has been created in the last 10 days" if($c eq 10);
}
...
while(<FH>) {
  if(/(PF\d\d\d\d\d)\s?\:? (\S+).+?(\S+)\/(\d+)\-(\d+) \((\S+) bits\).+(PF\d\d\d\d\d) (\S+) \S+\/(\d+)\-(\d+) \((\S+) bits\)/) {
