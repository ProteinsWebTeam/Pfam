...
if($remove_from_clan and $add_to_clan) {
    print STDERR "ERROR: You cannot use the -add_family and -remove_family options together\n";
}


if($add_to_clan) {
    unless($add_to_clan =~ /CL\d\d\d\d/) {
      print STDERR "ERROR: The clan accession you specified [$add_to_clan] is not in the correct format.\n";
      help()
    }
    ...
    unless($remove_from_clan =~ /CL\d\d\d\d/) {
      print STDERR "ERROR: The clan accession you specified [$remove_from_clan] is not in the correct format.\n";
      help()
    }
    print STDERR "Performing overlap checks for removing $family from $remove_from_clan\n";
}
...
my $pfamDB = Bio::Pfam::PfamDBManager->new( 'database' => "pfamlive", 
                                            'host' => 'pfamdb2a', 
                                            'port' => 3303, 
                                            'user' => 'pfam', 
                                            'password' => 'mafp1' );
...
if($rdb_family) {
    ...
    if($clan_result) {
        ...
        if($add_to_clan and $rdb_clan_acc) {         
            die "Cannot add $family to $add_to_clan.  $family already belongs to $rdb_clan_acc.\n";
        }        
        elsif($remove_from_clan and $rdb_clan_acc ne $remove_from_clan) {  
            die "$family belongs to $rdb_clan_acc and not to $remove_from_clan.  Incorrect clan accession entered on command line.";
        } 
        my $num_in_clan = @$clan_membership;
        if($remove_from_clan and $num_in_clan == 2) {
            warn "Cannot remove $family from $rdb_clan_acc because there are only two families in this clan and a clan must contain a minimum of two families\n";
   }
...
if (-e "$family/DESC")   {
   ...
   if (/^NE\s+(PF\d\d\d\d\d)/) {
            ...
            push(@ignore, $nested->pfamA_id) unless($ignore{$nested->pfamA_id});
...
if($add_to_clan or ($rdb_clan_acc and !$remove_from_clan)) {
    foreach my $clan_mem (@$clan_membership) {
        push (@ignore, $clan_mem->pfamA_id) unless($ignore{$clan_mem->pfamA_id});
    }
    $overlaps = &Bio::Pfam::PfamQC::family_overlaps_with_db($family,\@ignore,$endpoints_opt);
    warn "$family: found $overlaps external overlaps\n";


    #Need to also check for clan seed overlaps
    $clan_seed_overlap = Bio::Pfam::Clan::ClanQC::check_seed_overlap($pfamDB, $family, $clan_membership);   
