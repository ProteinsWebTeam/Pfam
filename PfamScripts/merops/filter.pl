...
#Ignore domains that are outcompeted, if any
foreach my $fam (@families)  {
  ...
  my %outcompeted;
  if(-s "$fam/outcompeted.txt") {
    open(OUTCOM, "$fam/outcompeted.txt") or die "Couldn't open fh to $fam/outcompeted, $!";
    while(<OUTCOM>) {
      if(/^\S+ \S+ \((\S+\/\d+-\d+)\,/) {
         $outcompeted{$1}=1; 
       }
       else {
         print STDERR "Failed to parse this line in $fam/outcompeted.txt:\n$_";
       }
    }
    close OUTCOM;
  }
  ...
  my (%sequences, %alignment, %seed, %uniquePattern, %asPattern, %acc_se);
  my $align="$fam/ALIGN";
  ...
  while(<ALIGN>) {
    if(/^((\S+)\/(\d+)-(\d+))\s+(\S+)/) {
      ...
      my $short_acc_se;
      if($acc_se =~ /[tr|sp]\|(\S+)\|\S+/) {
        $short_acc_se="$1/$start-$end"
      }
      else {
        $short_acc_se="$acc/$start-$end";
      }

      next if(exists($outcompeted{$short_acc_se})); #Ignore domains that are outcompeted
     
      $short_acc_se=sprintf('%-30s', $short_acc_se); #Uniprot acc can be 10 char, so 30 characters for the acc/start-ends should give plenty of room
      ...
      if($acc =~ /^H\|/) { #It's a holotype seq
        ...
        $alignment{$acc_se}=$short_acc_se."$seq\n";
      }
    }
    elsif(/^\/\//) {
    }
  ...
  #Sequences that pass filter go into a file called homologues.aln
  ...
  foreach my $acc_se (keys %sequences) { 
    ...
    if(-s "$fam/activeSite.dat") { #There is active site data, so look for the presence of the active site pattern
      ...
      #It's passed the length filter, and there are no active site data
      print HOMOLOGUES $alignment{$acc_se}
    }
  }
  close ADDITIONAL;
  ...
  while ($sth->fetch()) {
    ...
    if($as and $acceptAs) {
      ...
      for(my $i=0; $i<@asPos; $i++) {
        ...
        unless($resNum == 0 or ($resNum < $start or $resNum > $end)) { #Unless residue is 0 (we don't know the position) or the residue falls outside the alignment
    ...
    if($fam =~ /^M/) { #It's a metallopeptidase, so need to look for metal binding sites too
      if($metLig and $acceptMetLig) {
        ...
        for(my $i=0; $i<@metPos; $i++) {
          ...
          unless($metResNum == 0 or ($metResNum < $start or $metResNum > $end) ) { #Unless residue is 0 (we don't know the position) or the residue falls outside the alignment
