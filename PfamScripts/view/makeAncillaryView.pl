...
if(! $proteomeView->statusCheck('doneVersion')){
    ...
    $proteomeView->touchStatus('doneVersion');
...
#         system("bsub -q $queue -R \"rusage[mem=32000]\" -M 32000 -o $touch_dir/arch3.log 'make_Architecture_new_part3.pl && touch $touch_dir/doneArch'") and die $logger->logdie("Can't run make_Architecture_new_part3.pl, $!");
#         my $x=0;
#         until($x==1) {
#           sleep(600);
...
### pfamA_ncbi table is now populated on family ViewProcess
