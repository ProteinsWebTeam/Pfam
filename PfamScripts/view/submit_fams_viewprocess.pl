...
foreach my $fam_acc (@fam_list) {
  ...
  if ($num_seqs > 1800000) {
    $mem = "64G";
    $time = "4-00:00:00";
  } elsif ($num_seqs > 900000) {
    $mem = "32G";
    $time = "2-00:00:00";
  } elsif ($num_seqs > 100000) {
    $mem = "16G";
    $time = "1-00:00:00";
  } elsif ($num_seqs > 20000) {
    $mem = "8G";
    $time = "8:00:00";
  ...
  # system "sbatch --time=${time} --mem=${mem} -o \"${uid}log\" --job-name=famQ${cur_q} --dependency=singleton --wrap='mkdir -p ${pwd}/${uid}/${fam_id} && cd ${pwd}/${uid}/${fam_id} && makepfamview_pointrel.pl -id ${uid} -family ${fam_id}'";
