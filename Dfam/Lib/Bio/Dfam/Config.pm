...
Bio::Dfam::Config - Provides all of the configuration that you need to run Dfam

=cut

package Bio::Dfam::Config;
...
=head2 new 
  ...
  Title    : new
  Usage    : Either my $config = Bio::Pfan::Config->new or my $config = new Bio::Dfam::Config
  Function : Reads in the config file (location set by the environment varible PFAM_CONFIG
  Args     : None
  Returns  : Bio::Dfam::Config object
...
=head2 location
  ...
  Function : Returns the locations where we are running dfam
...
=head2 dfamseqLoc
  ...
  Title    : dfamseqLoc
  Usage    : $config->dfamseqLoc
  Function : Returns the location of in dfamseq
  Args     : None - read only operator
  Returns  : string containing path to dfamseq    
...
sub dfamseqLoc {
  ...
  return $self->{dfamseq}->{location};
}

=head2 dfamseqFarmLoc

  Title    : dfamseqFarmLoc
  Usage    : $config->dfamseqFarmLoc
  Function : Returns the location of dfamseq on the farm nodes
  Args     : None - read only operator
  Returns  : string containing path to dfamseq on the farm nodes
...
sub dfamseqFarmLoc {
  ...
  return $self->{dfamseq}->{farmLocation};
}


=head2 dfamseqLustreLoc

  Title    : dfamseqLustreLoc
  Usage    : $config->dfamseqLustreLoc
  Function : Returns the location of dfamseq on lustre
  Args     : None - read only operator
  Returns  : string containing path to dfamseq on lustre
  
=cut

sub dfamseqLustreLoc {
  my $self = shift;
  if ( $#_ >= 0 ) {
    warn "Passed variable to ro config\n";
  }
  return $self->{dfamseq}->{lustreLocation};
}
...
=head2 dbsize
  ...
  Title    : dbsize
  Usage    : $config->dbsize
  Function : Returns the number of sequences in dfamseq
  Args     : None - read only operator
  Returns  : integer    
...
sub dfamjobs {
  ...
  return $self->{Model}->{DfamJobs};  
  
}

=head2 dfamOldRelAdmin 

  Title    : dfamOldRelAdmin
  Usage    :  
  Function :
  Args     :
  Returns  :
